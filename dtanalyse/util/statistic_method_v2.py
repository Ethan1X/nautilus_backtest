...
class DataInfo(object):
    ...
    def __init__(self, data, ts_event):
        self.data = data
        self.ts = ts_event

    def __str__(self):
        return f'[{self.data}, {self.ts}]'
        

# 在一个有序数组内定位
def locate_data(data, data_list, direct=1):
    l, r = 0, len(data_list) - 1
    pos = -1
    while l <= r:
        mid = (l + r) // 2
        if data_list[mid].data == data:
            pos = mid
            break
        if direct > 0:
            # 升序队列
...
# 在排序队列中添加数据
# def add_data(data, data_list, direct):
#     # direct > 0: 升序队列; 否则：降序队列
#     data_size = len(data_list)
#     if data_size == 0:
#         data_list.append(data)
#     else:
#         pos = locate_data(data.data, data_list, direct=1)
#         if pos == 0:
#             data_list = [data]
#         elif pos >= data_size:
#             data_list.append(data)
...
def add_data(data, data_list, direct=1):
    data_size = len(data_list)
    if data_size == 0:
        data_list.append(data)
    else:
...
def ts_to_second(ts, time_unit=int(1e9)):
    return int(ts) // time_unit

def get_second_shifted(ts, shift:int =1, time_unit=int(1e9)):
    _ts_in_s = ts_to_second(ts, time_unit)
    return _ts_in_s + int(shift)
