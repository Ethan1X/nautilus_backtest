...
class LoadS3Data:
    ...
    @staticmethod
    def get_cex_trade(begin_time, end_time, exchange, symbol, is_adj_time=True,is_merge=False):
        '''
    ...
    @staticmethod
    def get_cex_depth_online(begin_time, end_time, exchange, symbol):
        '''
            单独保存的 线上交易时使用的depth数据
        '''
        cex_depth = []
        begin_time_ = begin_time
        s3_dt = dt2str(begin_time_, DATETIME_FORMAT4)
        ...
        while s3_dt <= dt2str(end_time, DATETIME_FORMAT4):
            logging.info(f"开始恢复 {exchange}_{symbol} 的depth数据")
            depth_list = LoadS3Data.get_online_depth(exchange, symbol, s3_dt)
            cex_depth.extend(depth_list)
            begin_time_ += datetime.timedelta(hours=1)
            s3_dt = dt2str(begin_time_, DATETIME_FORMAT4)
        res = query_dict_list(cex_depth, 'time', gte=dt2unix(begin_time, 1000), lte=dt2unix(end_time, 1000))

        return res

    @staticmethod
    def get_online_depth(s3_exchange, symbol, s3_dt):
        depth_list = []
        depth_ret = get_data_frombucket(BUCKET_LIST[0], s3_dt, s3_exchange, symbol, cache=LoadS3Data.cache)
        if len(depth_ret) != 0:
            last_t = 0
            depth_one = {}

            for row in depth_ret:

                if last_t != row['tp']:
                    last_t = row['tp']
                    if len(depth_one) != 0:
                        depth_list.append(depth_one)
                    depth_one = {'time':row['tp'], 'asks':[], 'bids':[], 'tp':row['tp']}

                if row['t'] == 'sell':
                    depth_one['asks'].append({'p': row['p'], 's':row['s']})
                else:
                    depth_one['bids'].append({'p': row['p'], 's':row['s']})
