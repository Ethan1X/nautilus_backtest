...
def generate_date_list(start_date: str, end_date: str):
    _date_list = []
    _start_time = datetime.datetime.strptime(start_date, "%Y%m%d%H")
    _end_time = datetime.datetime.strptime(end_date, "%Y%m%d%H")

    _cur_time = _start_time
    while _cur_time <= _end_time:
        _cur_date = _cur_time.strftime("%Y%m%d%H")
        _date_list.append(_cur_date)
        _cur_time = _cur_time + datetime.timedelta(hours=1)
