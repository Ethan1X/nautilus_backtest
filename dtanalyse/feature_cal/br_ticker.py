...
from fm_ticker import start_time, end_time

'''
Simple backtest process
'''
catalog = ChronoDataCatalog("/data/catalog", show_query_paths=False)
START_BALANCE = ["0 SOL", "0 OP", "0 FIL", "0 MATIC", "100000 USDT"]
EXCHANGE = "BINANCE"

# instrument = catalog.clone_instrument("/data/catalog", CurrencyPair, "SOLUSDT.BINANCE", maker_fee="-0.001", taker_fee="0.002")
# instrument = catalog.clone_instrument("/data/catalog", CurrencyPair, "OPUSDT.BINANCE", maker_fee="-0.001", taker_fee="0.002")
...
instrument = catalog.clone_instrument("/data/catalog", CryptoPerpetual, "SOLUSDT.BINANCE_PERP", rewrite=True, maker_fee="-0.001", taker_fee="0.002")
# instrument = catalog.clone_instrument("/data/catalog", CryptoPerpetual, "OPUSDT.BINANCE_PERP", rewrite=True, maker_fee="-0.001", taker_fee="0.002")
...
strategies = [
    ImportableStrategyConfig(
        strategy_path="fm_ticker:FeatureManager",
        config_path="fm_ticker:FeatureManagerConfig",
        config=dict(
            instrument_id=instrument.id,
            data_mode="central",
