...
start_time = pd.Timestamp("2023-07-24 00:00:00", tz="HONGKONG")
end_time =  pd.Timestamp("2023-08-12 00:00:00", tz="HONGKONG")
month_str = "2023_0801_0831"
...
class FeatureManager(Strategy):
    ...
    def __init__(self, config: FeatureManagerConfig) -> None:
        ...
        # todo: depth, ticker
        self.interval_ms = 100           # 最小时间尺度，毫秒
        self.interval_unit = int(1e6)
    ...
    def set_trade_data(self, first_time=False):
        ...
        for key, gen in self.feature_gen_list.items():
            if self.count > 1000 and self.count % 10000 == 0:
                print(f'distribute trade data: {key} {self.load_ts_num}, {self.count}, {self.trade_price_buy[-1]}, {self.trade_price_sell[-1]}')
    ...
    def on_start(self) -> None:
        ...
        self.subscribe_order_book_at_interval(self.instrument_id, book_type=BookType.L2_MBP, depth=5, interval_ms=self.interval_ms)

    def on_order_book(self, orderbook: OrderBook) ->None:
        _new_orderbook = trans_backtest_depth(orderbook)
        _ts_event = None
        if _new_orderbook.ts_event <= self.last_depth_ts:
            # 没有新数据，沿用老数据
            self.last_depth_ts += self.interval_ms * self.interval_unit
            _ts_event = self.last_depth_ts
            _new_orderbook = self.last_depth
        else:
            self.last_depth = _new_orderbook
            self.last_depth_ts = int(_new_orderbook.ts_event)
        # print(_new_orderbook.ts_event, _ts_event)
        self.process(_new_orderbook, 'depth', _ts_event)
    ...
    def process(self, data, data_type, ts=None):
        if ts is None:
        ...
        for key, gen in self.feature_gen_list.items():
            ...
            if data_type == 'ticker':
                ...
                res = gen.process(depth=data, ts_event=ts_event)
            else:
